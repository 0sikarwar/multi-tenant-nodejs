...
const register = async (email, password, name, tenant_id, roleName) => {
    const hashedPassword = await bcrypt.hash(password, 10);
    const user = await userService.createUser(email, hashedPassword, name, tenant_id);
    ...
    return userService.getUserById(user.USER_ID);
};
